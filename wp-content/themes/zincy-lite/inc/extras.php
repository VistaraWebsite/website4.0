...
function zincylite_scripts() {
	...
	wp_enqueue_style( 'zincylite-font-awesome', get_template_directory_uri() . '/css/font-awesome.min.css',array(),'4.6.1' );
...
function zincy_lite_bxslider_cb(){
	...
	$a = get_theme_mod('slider_pager_option','yes');
	if($a == 'yes') {$a='true'; }else{ $a='false';}

	$b = get_theme_mod('slider_control_option','yes');
	if($b == 'yes') {$b='true'; }else{ $b='false';}
	
	$c = get_theme_mod('slider_transition','slide');
	if($c == 'yes') {$c='horizontal'; }else{ $c='fade';}
	
	$d = get_theme_mod('slider_auto_transition','yes');
	if($d == 'yes') {$d='true'; }else{ $d='false';};
	
	$e = get_theme_mod('slider_speed','3000');
	$f = get_theme_mod('slider_pause','4000');
...
/** Plugin Install ***/
function zincy_lite_required_plugins() {
...
$plugins = array(
	...
	*/
	$config = array(
		'default_path' => '',
		'menu'         => 'zincy-lite-install-plugins',
		'has_notices'  => true,
		'dismissable'  => false,
		'dismiss_msg'  => '',
		'is_automatic' => true,
		...
		'strings'      => array(
			...
			'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.','zincy-lite' ),
			'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.','zincy-lite' ),
			'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.','zincy-lite' ),
			'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.','zincy-lite' ),
			'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.','zincy-lite' ),
			'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.','zincy-lite' ),
			'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.','zincy-lite' ),
			'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.','zincy-lite' ),
			'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins','zincy-lite' ),
			'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins','zincy-lite' ),
			'return'                          => __( 'Return to Required Plugins Installer', 'zincy-lite' ),
			'plugin_activated'                => __( 'Plugin activated successfully.', 'zincy-lite' ),
			'complete'                        => __( 'All plugins installed and activated successfully. %s', 'zincy-lite' ),
			'nag_type'                        => 'updated'
			)
);
...
add_action( 'tgmpa_register', 'zincy_lite_required_plugins' );
